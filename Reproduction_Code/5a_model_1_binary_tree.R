...
# Predicting P(y = 1) from an rpart fit
predict_prob <- function(fit, X) {
  df <- as.data.frame(X)
  p  <- predict(fit, newdata = df, type = "prob")
  # The "1" column is the positive-class probability
...
sort_params <- c("min_impurity_decrease",
...
final_params <- chosen[c("min_impurity_decrease",
                         "min_weight_fraction_leaf",
                         "max_depth",
                         "min_samples_split")]
