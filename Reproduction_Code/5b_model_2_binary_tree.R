...
upsample         <- FALSE   # model_2 has no upsampling
...
fit_tree <- function(X, y, params) {
  ...
  n <- nrow(df)
  # taking the larger leaf size
  frac_leaf <- max(1L, ceiling(params$min_weight_fraction_leaf * n))
  minbucket <- max(params$min_samples_leaf, frac_leaf)

  ctrl <- rpart::rpart.control(
    minsplit  = max(2L, 2L * minbucket),
    minbucket = minbucket,
...
# Tuning grid for model_2's three influential hyperparameters
grid <- expand.grid(
  min_impurity_decrease    = c(0, 0.001, 0.005, 0.01, 0.02, 0.05, 0.1),
  min_samples_leaf         = c(1, 2, 5, 10),
...
# Most parsimonious within one SE of the best mean AUC
