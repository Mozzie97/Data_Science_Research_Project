...
fit_tree <- function(X, y, params) {
  ...
  ctrl <- rpart::rpart.control(
    minsplit  = 2L,
    minbucket = max(1L, ceiling(params$min_weight_fraction_leaf * n)),
...
# Tuning grid for model_1b's three influential hyperparameters
grid <- expand.grid(
  min_impurity_decrease    = c(0, 0.001, 0.005, 0.01, 0.02, 0.05, 0.1),
  min_weight_fraction_leaf = c(0, 0.01, 0.05, 0.1, 0.2),
  max_depth                = c(3, 5, 8, 12, 20),
...
best_trial <- list(
  ...
  min_weight_fraction_leaf = trials_df$min_weight_fraction_leaf[best_idx],
...
# Sorting directly on columns
sort_params <- c("min_impurity_decrease", "min_weight_fraction_leaf", "max_depth")
candidates  <- candidates[order(
  candidates$min_impurity_decrease,
  candidates$min_weight_fraction_leaf,
...
chosen <- list(
  ...
  min_weight_fraction_leaf = candidates$min_weight_fraction_leaf[1],
...
final_params <- list(
  min_impurity_decrease    = chosen$min_impurity_decrease,
  min_weight_fraction_leaf = chosen$min_weight_fraction_leaf,
