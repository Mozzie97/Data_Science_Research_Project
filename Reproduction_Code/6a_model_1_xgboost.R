# Purpose:
#   Tune and cross-validate an XGBoost classifier for model_1 (face mask
#   wearing, full training set, no upsampling). This R script condenses the
#   four-stage Python pipeline (6aa-6ad) into a single file:
#     1. Hyperparameter importance was identified in the original Python
#        Optuna runs (see 6aa_model_1_xgboost_hyperparamter_importance.py
#        header). The influential hyperparameters for this model were:
#          - learning_rate
#          - subsample
#          - max_depth
#          - colsample_bytree
#        We take these as given and skip rediscovery.
#     2. Tune the four important hyperparameters via a grid of combinations,
#        scoring each by mean ROC AUC over five stratified shuffle-split folds
#        (matching 4_logistic_regression.R's CV protocol).
#     3. Apply the most-parsimonious-within-one-SE rule from the paper
#        (Section 2.5.1 of rsos.241941): keep all trials within one standard
#        error of the best mean AUC, then sort by the influential parameters
#        in importance order and pick the smallest tuple.
#     4. Final 5-fold CV with the chosen hyperparameters, recording all five
#        metrics (precision, recall, ROC AUC, accuracy, F1).
#
#   Design notes:
#   * The Python source uses xgb.XGBClassifier. We use the xgboost R package,
#     which wraps the same C++ backend and accepts the same hyperparameter
#     names (eta = learning_rate, etc.). All non-tuned settings match the
#     Python defaults: n_estimators (nrounds) = 250, objective =
...
upsample         <- FALSE          # model_1 has no upsampling
n_estimators     <- 250            
...
# Predicting P(y = 1) from an xgboost fit
...
# Scoring one hyperparameter combination by 5-fold mean AUC + per-fold SE
...
# Tuning grid for model_1's four influential hyperparameters
grid <- expand.grid(
...
# Final 5-fold CV with the chosen hyperparameters, all five metrics
cv_scores <- list(
  fold           = integer(0),
