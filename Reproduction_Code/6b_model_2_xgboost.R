...
# Tuning grid for model_2's two influential hyperparameters
grid <- expand.grid(
  learning_rate = c(0.02, 0.05, 0.1, 0.15, 0.2, 0.3, 0.5, 0.7),
  subsample     = c(0.4, 0.55, 0.7, 0.85, 1.0),
...
for (k in seq_len(nrow(grid))) {
  ...
  cat(sprintf("  trial %3d/%d  eta = %.3f  subsample = %.2f  AUC = %.4f\n",
              k, nrow(grid), params$learning_rate, params$subsample, res$value))
}
...
sort_params <- c("learning_rate", "subsample")
